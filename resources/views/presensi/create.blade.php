...
<body class="bg-gray-100">
    <nav class="bg-white border-gray-200 dark:bg-gray-900 border-b-2 fixed z-50 top-0 w-full">
        <div class="max-w-screen-xl flex flex-wrap items-center mx-auto p-4">
            ...
            <div class="hidden w-full ml-8 mt-1 md:block md:w-auto" id="navbar-default">
                ...
                <li>
                    <a href="#" class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent">Substitute teacher</a>
                </li>
                <li>
    ...
    <form id="presenceForm" enctype="multipart/form-data" class="mt-2">
        @csrf
    ...
    <script>
        $(document).ready(function() {
            $('#presenceForm').on('submit', function(e) {
                e.preventDefault();
                $.ajax({
                    url: 'http://localhost:8000/api/presensis',
                    method: 'POST',
